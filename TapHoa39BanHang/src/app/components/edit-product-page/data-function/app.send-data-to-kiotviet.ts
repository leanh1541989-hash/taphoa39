...
export class SendDataToKiotVietService {
    ...
    async sendSingleGroupData(masterProduct: any, childProducts: any[]): Promise<any> {
        ...
        // Get fresh payload from KiotViet API for THIS master product
        const payload = await this.kiotvietService.getRequestBody(masterProduct.Id);
        ...
        // Create map of edited products for this group
        ...
        // Also create Id-based map for more reliable matching
        ...
        // Update main product (master) fields
        ...
        if (masterMatch) {
            ...
            if (masterMatch.Name && masterMatch.Name !== payload.Product.Name) {
                console.log(`🔄 [SendSingleGroup] Updating Name: ${payload.Product.Name} → ${masterMatch.Name}`);
                payload.Product.Name = masterMatch.Name;
                payload.Product.CompareName = oldName;
            }

            // CRITICAL: Use BasePrice if user changed it, otherwise use FinalBasePrice
            let basePriceToUse: number;
            if (basePriceChanged) {
                basePriceToUse = masterMatch.BasePrice;
            ...
            console.log('✏️ [SendSingleGroup] Updated main product fields:', {
                Code: payload.Product.Code,
                Name: payload.Product.Name,
                BasePrice: payload.Product.BasePrice,
                Cost: payload.Product.Cost,
        ...
        if (Array.isArray(payload.Product.ProductUnits)) {
            payload.Product.ProductUnits = payload.Product.ProductUnits.map((unit: any) => {
                // Find matching edited product by Id first (most reliable), then by Code
                ...
                if (match) {
                    ...
                    // Update Name if changed
                    if (match.Name && 'Name' in unit && match.Name !== unit.Name) {
                        console.log(`🔄 [SendSingleGroup] Updating unit Name: ${unit.Name} → ${match.Name}`);
                        const oldUnitName = unit.Name;
                        unit.Name = match.Name;
                        if ('CompareName' in unit) {
                            unit.CompareName = oldUnitName;
        ...
        console.log('🚀 [SendSingleGroup] Final payload being sent to KiotViet for product Id:', payload.Product.Id);

        // Send to KiotViet API
        const response = await this.kiotvietService.updateProductToKiotviet(payload);
