...
export class MainPageComponent implements OnInit, OnDestroy, DoCheck, AfterViewInit {
  ...
  async reload(): Promise<void> {
    ...
    try {
      ...
      // ✅ Log sample OnHand & OnHandNV từ Firebase để verify
      const firebaseSample = firebaseProducts.slice(0, 3);
      console.log('🔍 Sample OnHand & OnHandNV từ Firebase:', firebaseSample.map(p => ({
        Id: p.Id,
        Code: p.Code,
        OnHand: p.OnHand,
        OnHandNV: p.OnHandNV
      ...
      // =============================
      // BƯỚC 4: Merge - ƯU TIÊN OnHand & OnHandNV TỪ FIREBASE
      // =============================
      console.log('\n💾 BƯỚC 4: Merge và lưu IndexedDB...');

      // ✅ QUAN TRỌNG: Tạo map OnHand & OnHandNV từ Firebase trước
      ...
      for (const p of firebaseProducts) {
        if (p?.Id != null) {
          ...
          if (p.OnHandNV !== undefined && p.OnHandNV !== null) {
            ...
            if (Number.isFinite(onHandNV)) {
              firebaseOnHandNVMap.set(p.Id, onHandNV);
            }
          }
        }
      }
      console.log(`📊 Firebase OnHand map: ${firebaseOnHandMap.size} products`);
      console.log(`📊 Firebase OnHandNV map: ${firebaseOnHandNVMap.size} products`);

      // Merge products: lấy data từ KiotViet, nhưng OnHand & OnHandNV từ Firebase
      ...
      for (const p of kiotvietProducts) {
        if (p?.Id) {
          ...
          // ✅ GHI ĐÈ OnHandNV bằng giá trị từ Firebase (nếu có)
          const firebaseOnHandNV = firebaseOnHandNVMap.get(p.Id);
          if (firebaseOnHandNV !== undefined) {
            productCopy.OnHandNV = firebaseOnHandNV;
          }
      ...
      // ✅ Log sample merged OnHand & OnHandNV để verify
      const mergedSample = mergedProducts.slice(0, 3);
      console.log('🔍 Sample OnHand & OnHandNV sau merge:', mergedSample.map(p => ({
        Id: p.Id,
        Code: p.Code,
        OnHand: p.OnHand,
        OnHandNV: p.OnHandNV
      ...
      // ✅ Verify OnHand & OnHandNV trong IndexedDB
      const verifyProducts = await this.productService.getAllProductsFromIndexedDB();
      const verifySample = verifyProducts.slice(0, 3);
      console.log('🔍 Sample OnHand & OnHandNV trong IndexedDB sau reload:', verifySample.map(p => ({
        Id: p.Id,
        Code: p.Code,
        OnHand: p.OnHand,
        OnHandNV: p.OnHandNV
 ...
 */
  private async forceUpdateIndexedDB(products: Product[], preserveOnHand: boolean = false): Promise<void> {
    ...
    try {
      ...
      // ✅ Log sample để verify OnHand & OnHandNV
  ...
  onMenuClick(type: string) {
    ...
    } else if (type === 'danhsachnhanvien') {
      this.saveDataToLocalStorage();
      this.router.navigate(['/employee-list']);
    } else if (type === 'lichlamviec') {
      this.saveDataToLocalStorage();
      this.router.navigate(['/work-schedule']);
    } else if (type === 'bangchamcong') {
      this.saveDataToLocalStorage();
  ...
  openAddWorkerDialog() {
    ...
    dialogRef.afterClosed().subscribe((result: any) => {
      if (result) {
        ...
        // TODO: Save worker to database
        alert('Đã thêm nhân viên: ' + result.hoTen);
      }
    });
  }
